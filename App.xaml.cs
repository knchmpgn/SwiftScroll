...
namespace SwiftScroll
{
    ...
    {
        private const string SingleInstanceMutexName = "Global\\SwiftScroll_SingleInstance";
        ...
        {
            // Enforce single-instance execution using a global named mutex
            _singleInstanceMutex = new Mutex(initiallyOwned: true, name: SingleInstanceMutexName, createdNew: out _ownsMutex);
            if (!_ownsMutex)
            {
                System.Windows.MessageBox.Show("SwiftScroll is already running.", "SwiftScroll", MessageBoxButton.OK, MessageBoxImage.Information);
                Shutdown();
                return;
            }
