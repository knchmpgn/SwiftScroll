...
public sealed class SwiftScrollEngine : IDisposable
{
    ...
    public SwiftScrollEngine(AppSettings settings) => ApplySettings(settings);
    ...
    public void Start()
    {
        lock (_lock)
        {
            if (_running) return;
            _running = true;
            _thread = new Thread(Worker) { IsBackground = true, Name = "SwiftScrollEngine" };
            _thread.Start();
        }
    }
